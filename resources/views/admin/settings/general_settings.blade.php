        ...
        <div class="col-md-10 col-xs-12">
            ...
            <form action="{{ route('save_settings') }}" method="post"> @csrf
                ...
                <div class="form-group row {{ $errors->has('site_name') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="site_name"
                            ...
                            placeholder="@lang('admin.site_name')">
                ...
                <div class="form-group row {{ $errors->has('site_title') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="site_title"
                            ...
                            name="site_title" placeholder="@lang('admin.site_title')">
                ...
                <div class="form-group row {{ $errors->has('email_address') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="email_address"
                            ...
                            name="email_address" placeholder="@lang('admin.email_address')">
                    ...
                    <div class="col-sm-8 {{ $errors->has('default_timezone') ? 'has-error' : '' }}">
                        <select class="form-control select2" name="default_timezone" id="default_timezone">
                            ...
                            @foreach (timezone_identifiers_list() as $key => $value)
                                <option value="{{ $value }}" {{ $saved_timezone == $value ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                ...
                <div class="form-group row {{ $errors->has('date_format') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <fieldset>
                            ...
                            <label><input type="radio" value="F j, Y" name="date_format"
                                    {{ $saved_date_format == 'F j, Y' ? 'checked' : '' }}> {{ date('F j, Y') }}<code>F j,
                                    Y</code></label> <br />
                            <label><input type="radio" value="Y-m-d" name="date_format"
                                    {{ $saved_date_format == 'Y-m-d' ? 'checked' : '' }}>
                                {{ date('Y-m-d') }}<code>Y-m-d</code></label> <br />

                            <label><input type="radio" value="m/d/Y" name="date_format"
                                    {{ $saved_date_format == 'm/d/Y' ? 'checked' : '' }}>
                                {{ date('m/d/Y') }}<code>m/d/Y</code></label> <br />

                            <label><input type="radio" value="d/m/Y" name="date_format"
                                    {{ $saved_date_format == 'd/m/Y' ? 'checked' : '' }}>
                                {{ date('d/m/Y') }}<code>d/m/Y</code></label> <br />

                            <label><input type="radio" value="custom" name="date_format"
                                    {{ $saved_date_format == 'custom' ? 'checked' : '' }}> Custom:</label>
                ...
                <div class="form-group row {{ $errors->has('time_format') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <fieldset>
                            <label><input type="radio" value="g:i a" name="time_format"
                                    {{ get_option('time_format') == 'g:i a' ? 'checked' : '' }}>
                                {{ date('g:i a') }}<code>g:i a</code></label> <br />
                            <label><input type="radio" value="g:i A" name="time_format"
                                    {{ get_option('time_format') == 'g:i A' ? 'checked' : '' }}>
                                {{ date('g:i A') }}<code>g:i A</code></label> <br />

                            <label><input type="radio" value="H:i" name="time_format"
                                    {{ get_option('time_format') == 'H:i' ? 'checked' : '' }}>
                                {{ date('H:i') }}<code>H:i</code></label> <br />

                            <label><input type="radio" value="custom" name="time_format"
                                    {{ get_option('time_format') == 'custom' ? 'checked' : '' }}> Custom:</label>
                    ...
                    <div class="col-sm-8">
                        ...
                        <select name="currency_sign" class="form-control select2">
                            @foreach (get_currencies() as $code => $name)
                                <option value="{{ $code }}" {{ $current_currency == $code ? 'selected' : '' }}>
                                    {{ $code }} </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row {{ $errors->has('currency_position') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        ...
                        <select name="currency_position" class="form-control select2">
                            <option value="left" @if ($currency_position == 'left') selected="selected" @endif>
                                @lang('admin.left')</option>
                            <option value="right" @if ($currency_position == 'right') selected="selected" @endif>
                                @lang('admin.right')</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row {{ $errors->has('logo_settings') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <fieldset>
                            <label><input type="radio" value="show_site_name" name="logo_settings"
                                    {{ get_option('logo_settings') == 'show_site_name' ? 'checked' : '' }}>
                                @lang('admin.show_site_name') </label> <br />
                            <label><input type="radio" value="show_image" name="logo_settings"
                                    {{ get_option('logo_settings') == 'show_image' ? 'checked' : '' }}> @lang('admin.show_image')
                            </label> <br />
                        </fieldset>
                    </div>
                </div>
                ...
                <div class="form-group row {{ $errors->has('allowed_file_types') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="allowed_file_types"
                            value="{{ old('allowed_file_types') ? old('allowed_file_types') : get_option('allowed_file_types') }}"
                            name="allowed_file_types" placeholder="@lang('admin.allowed_file_types')">
